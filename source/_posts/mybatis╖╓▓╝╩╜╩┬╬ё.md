title: mybatis分布式事务管理
date: 2016.01.17
categories: java #分类
tags: [java, mybatis, 分布式, 事务]
toc: true
---
  __mybatis__提供了数据访问框架.相对hibernate，它更轻量化，相对jdbc，它更人性化．是java系访问数据库的最佳框架．mybatis的最佳实践，是与spring协同工作．
　下面以本人github项目[https://github.com/njkfei/spring-jta-mybatis.git](https://github.com/njkfei/spring-jta-mybatis.git)进行说明。
 该项目基于spring jdbctemplate atomikos 构建本项目,旨在学习分布式事务管理。下列均有所涉及：
 * spring 提供框架管理功能。
 * mybatis提供数据访问框架
 * 全部采用注解驱动
 * atomikos提供分布式事务处理框架
基本思路是配置两个数据源，每个数据源使用atomikos提供的数据源接口，再配置相应的dao.最后再使用atomikos提供事务处理框架。
